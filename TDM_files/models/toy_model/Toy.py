...
def build_toy2(opt, dim, diag, dt, t0, mode='so'):
    ...
    if diag:
        ...
        if mode == 'so':
            ...
            input_dim = 2 * dim**2 + dim ** 2
            out_dim = int(dim ** 2)
    return ScoreNetwork(input_dim, out_dim, dt = dt, t0 = t0, hidden_dim = opt.model_hidden_dim, num_layers=opt.model_blocks)
...
class ScoreNetwork(torch.nn.Module):
    def __init__(self, input_dim, output_dim, dt, t0,
        ...
        super().__init__()
        act = 'sin'
        self.layer = MLP(num_layers, input_dim + 1, hidden_dim, output_dim, act)
    ...
    def forward(self, x, v, t):
        ...
        output = self.layer(torch.cat([u, t.unsqueeze(-1)], dim=-1))
        return output
