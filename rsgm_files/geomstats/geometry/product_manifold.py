...
from geomstats.geometry.product_riemannian_metric import ProductRiemannianMetric, ProductSameRiemannianMetric
...
class ProductSameManifold(Manifold):
    ...
    def grad_marginal_log_prob(self, x0, x, t, thresh, n_max):
        t = t.unsqueeze(-1) * torch.ones((*x0.shape[:-1], self.mul), device=x0.device)
        with torch.enable_grad():
            x.requires_grad_(True)
            logp = self._iterate_over_manifolds(
                "grad_marginal_helper",
                {"x0": x0, "x": x, "t": t, "thresh": thresh, "n_max": n_max},
            )
        x.requires_grad_(False)
        return logp

    def random_uniform(self, n_samples=1, device='cpu'):
        size = (n_samples, self.mul, self.manifold.dim + 1)
        samples = torch.randn(size, device=device)
        norms = gs.linalg.norm(samples, axis=-1)
        samples = gs.einsum("..., ...i->...i", 1 / norms, samples)   
        return samples.reshape((*samples.shape[:-2],-1))
    ...
    def random_walk(self, x, t):
        print("random walk")
        t = t[:, None] * torch.ones(self.mul)[None, :]
    ...
    def logdetexp(self, x, y, is_vector=False):
        return NotImplementedError(f'TODO')
