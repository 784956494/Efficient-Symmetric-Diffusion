...
model = Concat(2, [5, 512, 512, 512], 'sin')
score = TorusGenerator(model, embedding, 2, manifold)
optimizer = optim.Adam(score.parameters(), lr=2e-4)
# def model(y, t, context=None):
#     """Vector field s_theta: y, t, context -> T_y M"""    
...
# def init_model(train_ds):
#     t = torch.zeros((512, 1))
#     data, context = next(train_ds)
#     score = model(data, t, context)
#     params = score.parameters()
#     # schedule_fn = instantiate(cfg.scheduler)
#     optimizer = optim.Adam(params, lr=2e-4)
    
#     # scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=schedule_fn)
...
# wrapped_data = np.mod(data, radius)
data_tensor = torch.tensor(data, dtype=torch.float32).to(device)
mean_tensor = mean.to(device)
# wrapped_data_tensor = tor.exp(mean_tensor, data_tensor)
wrapped_data_tensor = data_tensor.remainder(2 * np.pi)
theta1 = wrapped_data_tensor[:, 0]
theta2 = wrapped_data_tensor[:, 1] 
x1 = torch.cos(theta1) 
y1 = torch.sin(theta1)  
x2 = torch.cos(theta2) 
y2 = torch.sin(theta2)  

wrapped_data_tensor = torch.stack((x1, y1, x2, y2), dim=1)
print(wrapped_data_tensor[:5])

from torch.utils.data import TensorDataset, DataLoader, random_split
dataset = TensorDataset(wrapped_data_tensor)
...
import sys
tbar = tqdm(
            range(train_state.step, 1000),
            total=1000 - train_state.step,
...
try:
    for _ in tbar:
        ...
        train_state, loss_val = train_step_fn(train_state, batch.to(device))
        step = train_state.step
        assert(not loss_val.isnan().any())
        assert(not loss_val.isinf().any())
        if (step % 100) == 0:
            print(loss_val)
    torch.save(train_state.model_state.state_dict(), 'rgsm.test.pt')
...
# model = Concat(2, [5, 64, 64, 64], 'sin')
# score = TorusGenerator(model, embedding, 2, Torus(2))
# states = torch.load('rgsm.test.pt')
# score.load_state_dict(states)

try:
    base = DefaultDistribution(manifold=manifold, flow=flow)
    pushforward = SDEPushForward(flow, base, diffeq='sde', transform=transform)
    sampler_kwargs = dict(N=100, eps=1e-3, predictor="GRW")
    sampler = pushforward.get_sampler(score, train=False, **sampler_kwargs)
    shape = (int(512 * 32),)
    x = sampler('cpu', shape)
except Exception as e:
    # Open a text file in write mode
    with open("error_log_2.txt", "w") as file:
        # Write the error message to the file
        file.write(str(e))
        # Optionally, write the traceback for more detailed info
        import traceback
